...
EOF;
        ...
        $comment_body = <<<EOF
You can Assignee me, when test is pass, I will merge this Pull_request auto        
...
EOF;
        ...
        (new KhsCI([$this->git_type.'_access_token' => GetAccessToken::getGitHubAppAccessToken((int) $rid)]))
            ->issue_comments
            ->create($repo_full_name, $pull_request_id, $comment_body);
    }
    ...
    public function pull_request_assigned(string $content)
    {
        ...
        Build::setAutoMerge($this->git_type, (int) $rid, (int) Env::get('CI_AUTO_MERGE_METHOD', 1), $commit_id, (int) $pull_number);

        // 创建一条评论

        $comment_body = <<<EOF
You already Assignee me, when test is pass, I will merge this Pull_request auto        
...
EOF;
        ...
        (new KhsCI([$this->git_type.'_access_token' => GetAccessToken::getGitHubAppAccessToken((int) $rid)]))
            ->issue_comments
            ->create($repo_full_name, $pull_number, $comment_body);
    }
